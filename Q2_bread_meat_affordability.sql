...
chleba_afford AS (																														-- plat / kg chleba
    ...
    FROM plat_start ps
    JOIN plat_end pe 
		ON ps.industry_branch_code = pe.industry_branch_code
    JOIN roky r 
		ON 1=1
    JOIN cena_chleba cch_start 
		ON cch_start.rok = r.min_rok
    JOIN cena_chleba cch_end 
		ON cch_end.rok = r.max_rok
  ),
...
mleko_afford AS (																														-- plat / kg chleba
    ...
    JOIN roky r 
		ON 1=1
    JOIN cena_mleka cm_start 
		ON cm_start.rok = r.min_rok
    JOIN cena_mleka cm_end 
		ON cm_end.rok = r.max_rok
  )
...
LEFT JOIN mleko_afford ml 
	ON ch.industry_branch_code = ml.industry_branch_code
LEFT JOIN czechia_payroll_industry_branch cpib 
	ON ch.industry_branch_code = cpib.code
